...
class CS_ResNet(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 drop_path_rate=0.0):
        super(CS_ResNet, self).__init__(init_cfg)
    ...
    def _make_stem_layer(self, in_channels, stem_channels):
        ...
        self.pooler = CompressiveDownsampler(in_channels=64,sample=16,kernel_size=16)
    ...
    def init_weights(self):
        super(CS_ResNet, self).init_weights()
    ...
    def train(self, mode=True):
        super(CS_ResNet, self).train(mode)
        self._freeze_stages()
        if mode and self.norm_eval:
            #self.zero_grad(set_to_none=True)  # set_to_none=True可以提高内存效率
            for m in self.modules():
